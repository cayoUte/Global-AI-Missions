...
RESPONSIBILITIES
- Read docs/agents/SHARED_CONTEXT.md (§2, §3, §5, §12), docs/product/PRODUCT.md, docs/product/DECISIONS_LOG.md (PD-024), docs/product/MAYA.md, docs/assessment/ASSESSMENT_SPEC.md, content/missions/the-last-train/items.json and docs/contracts/mission.schema.json.
...
- Propose story-minute costs per edge (defaults: correct 1, incorrect 3) and 2–3 rescue edges on later checkpoints where Maya can realistically help (a shortcut she knows, a stranger she talks to). The Story Graph Engineer checks them once with the simulator.
- Define 2–3 flags (for example lost_ticket, wrong_platform, befriended_guard) and the later scene variants they unlock, so different paths feel different.
...
- Definition of done:
  ...
  - A practised presenter completes the mission in 5 minutes or less (PD-024); a first-time learner in about 8–10 minutes of reading.
...
- Narrative and consequence scenes of 1–3 short lines (PD-024); checkpoint scenes may add the item's stimulus. Outside the items, keep readability around A2–B1 in natural British English.
