...
MISSION
Make parallel work possible and keep it coherent. In the first 45 minutes, scaffold the application and freeze the contracts every agent builds against. During the build, review integrations at each gate and resolve change requests. In the last hour, write the technical decisions document and the interview defense.
Timebox: 45 minutes at the start, 10 minutes per gate, 40 minutes at the end.

RESPONSIBILITIES
- Read docs/agents/SHARED_CONTEXT.md completely; it already contains the decided stack, API surface, content format, invariants and ownership. Also read docs/product/PRODUCT.md §5 and docs/product/DECISIONS_LOG.md: the contract must carry every field the screens need (World card states, Progress, Report).
...
- Freeze the content contracts: docs/contracts/items.schema.json, docs/contracts/mission.schema.json and docs/contracts/catalog.schema.json (JSON Schema, from §12), plus a command that validates the three content files against them.
- Freeze docs/contracts/api-contract.md: every endpoint in §8 with request and response shapes, status codes, auth and role requirements, idempotency rules, the StateView shape, the Report shape (including feedback_source), the World card shape (the six states and is_maya_pick), the Progress shape, GET /api/config, the AnswerRequest validation rules and the explicit list of fields forbidden while an attempt is open.
- Define the error model: the envelope { "error": { "code", "message", "details" } }, a table of domain error codes (INVALID_CREDENTIALS, UNAUTHENTICATED for a missing or expired cookie, NOT_FOUND, ATTEMPT_NOT_IN_PROGRESS, NODE_OUT_OF_SEQUENCE, CHECKPOINT_LOCKED, MISSION_NOT_FINISHED, FORBIDDEN_ROLE, VALIDATION_ERROR, RATE_LIMITED with Retry-After, INTERNAL_ERROR) and their HTTP statuses (401, 403, 404, 409, 422, 429, 500). /simulate with DEMO_MODE off returns NOT_FOUND.
...
- Ownership lives in SHARED_CONTEXT §11 (no separate file). Create docs/contracts/CHANGE_REQUESTS.md; triage requests during the build and record each decision and its impact.
...
- Closing: write docs/DECISIONS.md in Spanish, 2 pages at most (about 900–1,000 words), covering exactly the 8 required points:
  ...
  - Scaling from 100 to 50,000–100,000 students: stateless API behind a load balancer (the login rate limit is the one per-process piece of state in the MVP; at scale it moves to a shared store or the API gateway), connection pooling, read replicas, caching of immutable mission versions, pre-generated audio on a CDN, LLM calls moved to a queue with workers (the seam already exists: the coach runs after the grading commit), rate limits and cost caps, observability.
  ...
  - AI without vendor lock-in: CoachProvider port, adapters, versioned prompts, validated structured output, fallback, stored provider/model/prompt_version, an evaluation set to compare providers. Two lines pointing to docs/ai/AI_ARCHITECTURE.md for the brief's §7 vision (personalized feedback, accompaniment, reminders, learning assistance).
...
- docs/contracts/: items.schema.json, mission.schema.json, catalog.schema.json, api-contract.md, conventions.md, CHANGE_REQUESTS.md.
