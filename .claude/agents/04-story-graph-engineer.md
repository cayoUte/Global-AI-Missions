...
Timebox: 90 minutes. Start with a small fixture graph and switch to the real mission.json after G1.
...
- CLI: python -m app.engine.cli validate | calibrate --profile A2 | simulate --profile B1 --seed 7.
- Calibrate costs and rescue edges against the targets in §5 with the real mission; send any content change to the Narrative Designer through docs/contracts/CHANGE_REQUESTS.md.
- Unit tests:
...
- docs/engine/CALIBRATION.md: the endings distribution per profile over 1,000 runs, the total path count, min/max minutes, and the thresholds chosen and why.
- Definition of done:
  - pytest backend/tests/engine is green.
  - The validator passes on the real mission.
  - Calibration is within the targets, or the deviation is documented.
