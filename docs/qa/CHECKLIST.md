...
## 4. Delivery checklist (delivery-engineer completes; tick with date and initials)
...
- [ ] Demo data fresh: `make reset` right before the demo (Ana must be a first meeting; the rehearsal consumes her).
- [ ] Speech checked by a human on the demo browser (M-07); keep the fallback as the plan B.
- [ ] No open S1/S2 bugs in §3 (BUG-001 must be fixed or explicitly accepted by the human).
- [ ] DECISIONS.md ≤ 2 pages and covers the 8 points; INTERVIEW.md; AI_USAGE.md truthful; `docs/ai/AI_ARCHITECTURE.md` linked from the README's evaluator section.
- [ ] Demo rehearsal done with a stopwatch (M-13), ≤ 10 min.
