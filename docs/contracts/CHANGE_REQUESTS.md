...
### CR-011 — `/simulate` ambiguities resolved while implementing the stretch endpoint
- Requested by: backend-api-engineer · Date: 2026-09-25 · Status: done
- File(s): `docs/contracts/api-contract.md` §5.16 (implementation notes appended; the response shape is unchanged). Code: `backend/app/api/routers/simulation.py`, `backend/app/services/simulation.py`, `backend/app/schemas/simulation.py`, `backend/app/api/deps.py` (`require_demo_mode`).
- Change: (1) with `DEMO_MODE=false` the 404 comes before authentication and query validation, so the route looks unknown to everyone; (2) `profile` is required and `seed` is bounded to `0..1000000` (the contract only said "optional integer"); (3) an unknown or non-playable mission, or one without an active version, is 404, and unlock rules do not apply (any role); (4) `outcome` maps the engine's correct / incorrect to `understood` / `missed`; `clock` and `maya_mood` are on arrival, `maya_decision` is the StateView's arrival decision; (5) the trace's `item_id` is not returned (the contract's field list does not name it; safer reading of SHARED_CONTEXT §9.7).
- Why: the frozen text left these open; each choice is the narrower or safer one.
- Impact: frontend only through the regenerated types (`SimulationResponse`, `SimulationStep` in `frontend/src/api/schema.d.ts`); no screen uses the endpoint yet. Tests: `backend/tests/api/test_simulate.py`.
- Decision: done (within the frozen shape; the tech-lead may revisit the seed bound).
